        	...
        	if(node.id == 0){
        		...
        		$.post("/content/category/update",{id:node.id,name:node.text},function (data) {
                    if(data.status == 200 && data.data.flag == "1"){
                        $.messager.alert('提示',node.text+' 重命名成功!');
                    }else{
                        $.messager.alert('提示',node.text+' 重命名失败!');
                    }
                });
        	}
        }
		...
		$.messager.confirm('确认','确定删除名为 '+node.text+' 的分类吗？',function(r){
			if(r){
				$.post("/content/category/delete/",{id:node.id},function(data){
                    ...
                    }else if(data.status == 200 && data.data.flag == "1"){
                        $.messager.alert('提示',node.text+' 删除成功!');
                        tree.tree("remove",node.target);
                    } else {
                        $.messager.alert('提示',node.text+' 删除失败!');
                    }
